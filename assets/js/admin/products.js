...
class ProductsManager {
    ...
    async loadProducts() {
        try {
            const response = await supabaseRequest('products?select=*&order=name.asc');
    ...
    async loadCategories() {
        try {
            const response = await supabaseRequest('products?select=category&is_active=eq.true');
            const categories = [...new Set(response.map(p => p.category).filter(c => c))];
