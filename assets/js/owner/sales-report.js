...
class SalesReportManager {
    ...
    resetFilters() {
        ...
        this.filters = { dateFrom: lastMonth.toISOString().split('T')[0], dateTo: today.toISOString().split('T')[0], branchId: 'all' };
        this.loadData();
    ...
    async loadSales() {
        try {
            ...
            // 1. جلب المبيعات
            let query = 'daily_sales?select=id,branch_id,product_id,quantity,sale_date,is_closed&order=sale_date.desc';
            const filters = [];

            if (dateFrom) filters.push(`sale_date=gte.${dateFrom}`);
            if (dateTo) filters.push(`sale_date=lte.${dateTo}`);
            if (branchId && branchId !== 'all') filters.push(`branch_id=eq.${branchId}`);
            ...
            // 2. جلب أسماء الفروع
            const branchIds = [...new Set(sales.map(s => s.branch_id).filter(id => id))];
            let branchMap = {};
            if (branchIds.length > 0) {
                const branchQuery = branchIds.map(id => `id.eq.${id}`).join(',');
                const branches = await supabaseRequest(`branches?select=id,name&or=(${branchQuery})`);
...
function resetFilters() {
    if (salesReportManager) salesReportManager.resetFilters();
}
